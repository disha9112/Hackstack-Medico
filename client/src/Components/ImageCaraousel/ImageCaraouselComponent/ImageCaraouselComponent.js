...
const ImageCaraouselComponent = (props) => {
  let { image, isButtonThere, second, className, heading, children } = props;

  return (
    <div className={`${styles["image-caraousel-container"]} ${className}`}>
      <div className={`${styles["image-caraousel"]}`}>
        <div
          className={`${second ? styles["image-container"] : styles["no-image-container"]
            }`}
        >
          {image}
        </div>
        ...
        <div className={`${styles["content-container"]}`}>
          ...
          <div
            className={`${isButtonThere === true ? styles["button"] : styles["no-button"]
              }`}
          >
